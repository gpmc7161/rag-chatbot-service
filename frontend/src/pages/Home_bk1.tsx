        ...
        if (Array.isArray(result.rag_chatbot.references)) {
          sources = result.rag_chatbot.references.map((ref: any) => {
            if (typeof ref === 'string') {
              return { reference: ref, text: ref };
            }
            // 백엔드 형식: {chunk_id, similarity, preview}
            return {
              reference: String(ref.chunk_id !== undefined ? `청크 #${ref.chunk_id}` : (ref.reference || 'Unknown')),
              text: String(ref.preview || ref.text || ref.content || JSON.stringify(ref))
            };
          });
        }
      }
